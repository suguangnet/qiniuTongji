...
try:
    from qiniu.http import QiniuMacAuth
except ImportError:
...
class QiniuStorageUsage:
    def __init__(self, access_key, secret_key):
        ...
        self.base_url = "https://api.qiniuapi.com/v6"
    ...
    def _create_token(self, path, body=None):
        ...
        # 根据七牛云文档，管理API需要使用正确的签名方法
        # 直接使用token_of_request方法，这是官方推荐的方式
        token = self.auth.token_of_request(url=full_url)
        
        return f"Qiniu {token}"
    
    def _create_token_with_date(self, path, date_str, body=None):
        """
        创建包含日期信息的访问令牌（使用七牛云官方SDK）
        
        Args:
            path (str): 请求路径
            date_str (str): 日期字符串，格式为 YYYYMMDDTHHMMSSZ
            body (str, optional): 请求体内容
            
        Returns:
            str: 访问令牌
        """
        # 使用QiniuMacAuth来处理X-Qiniu-Date头部的签名
        # 这是七牛云管理API的正确签名方式
        full_url = f"https://api.qiniuapi.com{path}"
        ...
        # 构建包含X-Qiniu-Date的请求头信息
        qheaders = f"X-Qiniu-Date: {date_str}"
        
        # 使用QiniuMacAuth进行签名，包含X-Qiniu-Date头部
        token = self.mac_auth.token_of_request(
            method="GET",
            host=host,
            url=f"{url_path}?{query}" if query else url_path,
            qheaders=qheaders,
    ...
    def get_storage_usage(self, bucket_name=None, region=None, begin_time=None, end_time=None, granularity='day'):
        ...
        Returns:
            dict: 包含存储用量信息的字典
        """
        return self._make_api_request('space', bucket_name, region, begin_time, end_time, granularity)
    ...
    def get_file_count(self, bucket_name=None, region=None, begin_time=None, end_time=None, granularity='day'):
        ...
        Returns:
            dict: 包含文件数量统计信息的字典
        """
        return self._make_api_request('count', bucket_name, region, begin_time, end_time, granularity)
    ...
    def get_low_freq_storage_usage(self, bucket_name=None, region=None, begin_time=None, end_time=None, granularity='day'):
        ...
        Returns:
            dict: 包含低频存储量统计信息的字典
        """
        return self._make_api_request('space_line', bucket_name, region, begin_time, end_time, granularity)
    
    def _make_api_request(self, api_endpoint, bucket_name=None, region=None, begin_time=None, end_time=None, granularity='day'):
        """
        通用API请求方法
        
        Args:
            api_endpoint (str): API端点名称，如 'space', 'count', 'space_line' 等
            bucket_name (str, optional): 存储空间名称
            region (str, optional): 存储区域ID
            begin_time (str): 起始日期字符串
            end_time (str): 结束日期字符串
        ...
        # 生成时间戳，确保X-Qiniu-Date头与token生成使用相同的时间
        date_header = time.strftime('%Y%m%dT%H%M%S', time.gmtime()) + 'Z'
        
        # 创建访问令牌，将时间戳信息传递进去
        token = self._create_token_with_date(path, date_header)
...
<body>
    ...
    <script>
        ...
        document.getElementById('queryForm').addEventListener('submit', function(e) {
            ...
            const apiType = formData.get('api_type');  // 获取API类型
        ...
        function displayResults(data) {
            ...
            for (let i = 0; i < data.times.length && i < data.datas.length; i++) {
                ...
                const readableSize = formatBytes(sizeBytes);
...
def query_usage():
    try:
        ...
        # 根据API类型选择查询方法
        api_type = data.get('api_type', 'space')  # 新增：API类型参数，默认为space
        
        if api_type == 'space':
            result = qiniu_usage.get_storage_usage(
            ...
            result = qiniu_usage.get_file_count(
                bucket_name=bucket_name,
                begin_time=begin_time,
                end_time=end_time,
                granularity=granularity
            ...
            result = qiniu_usage.get_low_freq_storage_usage(
                bucket_name=bucket_name,
                begin_time=begin_time,
                end_time=end_time,
                granularity=granularity
